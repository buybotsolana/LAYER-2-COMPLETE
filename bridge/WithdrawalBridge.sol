...
/**
 * @title WithdrawalBridge
 * @dev Bridge specializzato per gestire i prelievi da Solana a Ethereum
 * Questo contratto gestisce i prelievi di token da Solana a Ethereum
 */
contract WithdrawalBridge is Ownable, ReentrancyGuard, Pausable {
     ...
     * @param _tokenBridge Indirizzo del TokenBridge
     */
    constructor(address _validator, address _tokenBridge) {
    ...
    /**
     * @dev Modifica per verificare che il chiamante sia il TokenBridge
     */
    modifier onlyTokenBridge() {
        require(msg.sender == tokenBridge, "Caller is not the token bridge");
        _;
    }
     ...
     * @param _minAmount Importo minimo per i prelievi
     * @param _maxAmount Importo massimo per i prelievi
     * @param _dailyLimit Limite giornaliero per i prelievi
     * @param _fee Commissione per i prelievi (in percentuale, con 2 decimali)
     */
    function addToken(
        address _token,
        uint256 _minAmount,
    ...
    function initiateWithdrawal(
        ...
        withdrawals[withdrawalId] = Withdrawal({
            ...
            sender: _sender,
            amount: amountAfterFee, // Salva l'importo dopo la commissione
        ...
        emit WithdrawalInitiated(withdrawalId, _token, _recipient, _sender, amountAfterFee);
    ...
    /**
     * @dev Imposta l'indirizzo del TokenBridge
     * @param _tokenBridge Nuovo indirizzo
     */
    ...
    /**
     * @dev Verifica se un prelievo è stato processato
     * @param _withdrawalId ID del prelievo
     * @return True se il prelievo è stato processato
     */
    function isWithdrawalProcessed(bytes32 _withdrawalId) external view returns (bool) {
        return withdrawals[_withdrawalId].processed;
    }

    /**
     * @dev Ottiene le informazioni di un prelievo
     * @param _withdrawalId ID del prelievo
     * @return token Indirizzo del token
     * @return recipient Indirizzo del destinatario
     * @return sender Indirizzo del mittente su Solana
     * @return amount Importo
     * @return timestamp Timestamp
     * @return transactionHash Hash della transazione su Solana
     * @return processed Flag che indica se il prelievo è stato processato
     */
    function getWithdrawalInfo(bytes32 _withdrawalId) external view returns (
        address token,
        address recipient,
        bytes32 sender,
        uint256 amount,
